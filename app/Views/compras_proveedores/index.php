...
<style>
    ...
    .proveedor-autocomplete-list {
        ...
        scrollbar-color: rgba(255,193,7,.3) transparent;
    }
    .proveedor-autocomplete-list.open {
        display: block;
    ...
    .proveedor-autocomplete-item:first-child { border-radius: 10px 10px 0 0; }
    .proveedor-autocomplete-item:last-child  { border-radius: 0 0 10px 10px; }
    .proveedor-autocomplete-item:hover,
    .proveedor-autocomplete-item.active {
        background: rgba(255, 193, 7, 0.15);
        color: #ffc107;
    }
    .proveedor-autocomplete-item .match-highlight {
        color: #ffc107;
        font-weight: 700;
    }
    .proveedor-autocomplete-empty {
        padding: 10px 14px;
        color: #666;
        font-size: .85rem;
        text-align: center;
    ...
    .prod-autocomplete { position: relative; }
    .prod-autocomplete-list {
        position: absolute;
        top: calc(100% + 4px);
        left: 0; right: 0;
        z-index: 1090;
        ...
        display: none;
        scrollbar-width: thin;
        scrollbar-color: rgba(99,179,237,.3) transparent;
    }
    .prod-autocomplete-list.open { display: block; animation: fadeInDown .15s ease; }
    .prod-autocomplete-item {
        padding: 8px 13px;
        cursor: pointer;
        font-size: .85rem;
        color: #d0d8e8;
    ...
    .prod-autocomplete-item:first-child { border-radius: 10px 10px 0 0; }
    .prod-autocomplete-item:last-child  { border-radius: 0 0 10px 10px; }
    .prod-autocomplete-item:hover,
    .prod-autocomplete-item.pa-active {
        background: rgba(99,179,237,.15);
        color: #63b3ed;
    }
    .prod-autocomplete-item .sku-tag {
        font-size: .75rem;
        color: #888;
        margin-left: 6px;
    }
    .prod-autocomplete-item .pm-highlight { color: #63b3ed; font-weight: 700; }
    .prod-autocomplete-item.libre-item {
        color: #ffc107;
        font-style: italic;
    ...
    /* Proveedor focus / validación */
    #proveedor-search-input:focus {
        box-shadow: 0 0 0 2px rgba(255,193,7,.35) !important;
        border-color: rgba(255,193,7,.6) !important;
    }
    #proveedor-search-input.is-invalid-custom {
...
<div class="container py-4">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
        <div>
            ...
            <p class="text-muted mb-0">Registra y administra las facturas de proveedores, prorratea costos y actualiza el stock del inventario.</p>
            <div class="admin-subtitle-line" style="background-color: #ffc107;"></div>
        </div>
        <div>
            <button type="button" class="btn btn-warning rounded-pill px-4 fw-bold d-inline-flex align-items-center gap-2 text-dark" data-bs-toggle="modal" data-bs-target="#modalNuevaCompra">
    ...
    <div class="card border-0 shadow-sm admin-card">
        <div class="card-header border-bottom border-secondary border-opacity-25 py-3 bg-transparent">
            <div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-3">
                ...
                <div class="d-flex align-items-center gap-3">
                    <h6 class="fw-bold mb-0 text-white"><i class="fas fa-file-invoice-dollar me-2 text-warning"></i> Historial de Facturas de Proveedores</h6>
                ...
                <form method="GET" action="<?= base_url('admin/compras') ?>" id="form-filtro-proveedor" class="d-flex align-items-center gap-2">
                    <div class="input-group input-group-sm" style="min-width: 240px; max-width: 340px;">
                        <span class="input-group-text border-0" style="background: rgba(255,255,255,0.07); color: #ffc107;">
                            <i class="fas fa-search"></i>
                        </span>
                        <input type="text"
                               name="busqueda"
                               id="live-search-proveedor"
                               ...
                               autocomplete="off">
                        <?php if ($busqueda !== ''): ?>
                        <a href="<?= base_url('admin/compras') ?>" class="btn btn-sm border-0 d-flex align-items-center px-2"
                    ...
                    <?php if (!empty($compras)): ?>
                        <?php foreach ($compras as $c): ?>
                            <tr class="admin-table-tr custom-row-glow">
                                ...
                                <td class="py-3 text-white-50 text-truncate" style="max-width: 250px;" title="<?= esc($c['descripcion']) ?>">
                                    <?= esc($c['descripcion'] ?: '-') ?>
                                </td>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="7" class="text-center py-5 text-muted">
                                ...
                                <p class="mb-0 small">Registra tus compras a proveedores para actualizar stock y egresos de Caja Chica.</p>
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
        ...
        ?>
        <div class="d-flex flex-column flex-sm-row justify-content-between align-items-center gap-3 px-4 py-3" style="border-top: 1px solid rgba(255,255,255,0.07);">
            <!-- Info de registros -->
            <small class="text-white-50">
                Mostrando
                <strong class="text-white"><?= number_format(($paginaActual - 1) * $porPagina + 1) ?></strong>
                –
                <strong class="text-white"><?= number_format(min($paginaActual * $porPagina, $totalRegistros)) ?></strong>
                ...
                <ul class="pagination pagination-sm mb-0 gap-1">
                    ...
                    <?php for ($p = $inicio; $p <= $fin; $p++): ?>
                        <li class="page-item <?= $p === $paginaActual ? 'active' : '' ?>">
                            <a class="page-link rounded-3 border-0 fw-bold"
                               href="<?= base_url('admin/compras') ?>?page=<?= $p ?><?= $qs ?>"
                    ...
                    <?php if ($fin < $totalPaginas): ?>
                        ...
                        <?php endif; ?>
                        <li class="page-item">
                            <a class="page-link rounded-3 border-0" href="<?= base_url('admin/compras') ?>?page=<?= $totalPaginas ?><?= $qs ?>"
...
<div id="loading-indicator" class="htmx-indicator position-fixed top-50 start-50 translate-middle p-3 bg-dark bg-opacity-75 rounded-3" style="z-index: 1200;">
...
<div class="modal fade" id="modalDetalle" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg" id="contenido-modal" style="background-color: #121824; color: #ffffff; border-radius: 20px; border: 1px solid rgba(255,255,255,0.1);">
...
<div class="modal fade" id="modalConfirmarEliminarCompra" tabindex="-1" aria-labelledby="modalConfElimCompraLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content modal-encargo-content text-white" style="border-radius: 20px; background-color: #121824; border: 1px solid rgba(255,255,255,0.15);">
            <div class="modal-header modal-encargo-header py-3 px-4" style="border-bottom: 1px solid rgba(255,255,255,0.1);">
                <h5 class="modal-title fw-bold text-white d-flex align-items-center gap-2" id="modalConfElimCompraLabel">
                    <i class="fas fa-exclamation-triangle text-danger"></i> ¿Eliminar registro de compra?
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-4 text-center">
                <p class="text-white-50" style="font-size: 0.95rem;">Esta acción realizará las siguientes operaciones:</p>
                ...
                <p class="mt-3 mb-0 text-warning fw-bold small"><i class="fas fa-exclamation-circle me-1"></i> Asegúrese de que el stock actual cubra la cantidad a restar para evitar stock en negativo.</p>
            </div>
            <div class="modal-footer d-flex gap-2 justify-content-center pb-4" style="border-top: none;">
                ...
                <form id="formConfirmarEliminarCompra" action="" method="POST" class="d-inline">
                    <?= csrf_field() ?>
...
<div class="modal fade" id="modalNuevaCompra" tabindex="-1" aria-labelledby="modalNuevaCompraLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content modal-encargo-content text-white" style="background-color: #121824; border-radius: 20px; border: 1px solid rgba(255,255,255,0.1);">
            <div class="modal-header modal-encargo-header py-3 px-4" style="border-bottom: 1px solid rgba(255,255,255,0.1);">
                <h5 class="modal-title fw-bold text-white d-flex align-items-center gap-2" id="modalNuevaCompraLabel">
                    <i class="fas fa-cart-plus text-warning"></i> Registrar Compra a Proveedor
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="formNuevaCompra" action="<?= base_url('admin/compras/crear') ?>" method="POST">
                <?= csrf_field() ?>
                <div class="modal-body p-4">
                    ...
                    <div class="row g-3 mb-4 p-3 rounded-4 compra-details-box">
                        ...
                        <div class="col-12 col-md-4">
                            <label for="proveedor-search-input" class="form-label modal-encargo-label">Proveedor *</label>
                        ...
                        <div class="col-12 col-md-4">
                            ...
                            <input type="date" class="form-control modal-encargo-control text-white" id="fechaCompra" name="fechaCompra" value="<?= date('Y-m-d') ?>" required>
                        </div>

                        <!-- Descripcion / Concepto -->
                        <div class="col-12 col-md-4">
                            <label for="descripcion" class="form-label modal-encargo-label">Descripción General / Comentario</label>
                            <input type="text" class="form-control modal-encargo-control text-white" id="descripcion" name="descripcion" placeholder="Ej: Compra de navidad, Lote de plumas, etc.">
                        </div>

                        <!-- Costos Adicionales -->
                        <div class="col-12 col-md-6">
                            <label for="envio_local_estimado" class="form-label modal-encargo-label">Envío Local o Envío Estimado ($)</label>
                            <input type="number" step="0.01" class="form-control modal-encargo-control text-white" id="envio_local_estimado" name="envio_local_estimado" min="0" value="0.00" onkeyup="recalcularMontos()" onchange="recalcularMontos()">
                        </div>
                        
                        <div class="col-12 col-md-6">
                            <label for="impuesto_importacion" class="form-label modal-encargo-label">Impuesto o Tarifa de Importación ($)</label>
                            <input type="number" step="0.01" class="form-control modal-encargo-control text-white" id="impuesto_importacion" name="impuesto_importacion" min="0" value="0.00" onkeyup="recalcularMontos()" onchange="recalcularMontos()">
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <button type="button" class="btn btn-outline-warning btn-sm rounded-pill px-4 fw-bold" onclick="agregarFila()">
                    ...
                    <div id="productos-container">
                        <!-- Las filas de productos se inyectarán con JS -->
                    </div>

                    <!-- Resumen del Pedido en el pie del modal -->
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 p-3 rounded-4 mt-4" style="background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.05);">
                        <div class="text-white-50 small">
                            <div>Total Base de Productos: <strong class="text-white">$<span id="total-productos-label">0.00</span></strong></div>
                            <div>Envío + Impuesto: <strong class="text-white">$<span id="total-adicionales-label">0.00</span></strong></div>
                        </div>
                        <div class="d-flex align-items-center gap-3">
                ...
                <div class="modal-footer modal-encargo-footer d-flex gap-2" style="border-top: 1px solid rgba(255,255,255,0.1);">
                    <button type="button" class="btn btn-outline-light rounded-pill px-4" data-bs-dismiss="modal">Cancelar</button>
                    <button type="submit" class="btn btn-warning rounded-pill px-4 fw-bold text-dark hover-shadow" id="btn-submit-compra">
...
(function() {
    ...
    window.agregarFila = function() {
        ...
        const filaHtml = `
        <div class="row-producto card border-secondary border-opacity-25 bg-transparent p-3 mb-3" style="border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; background: rgba(255,255,255,0.01) !important;" id="row_${index}">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h6 class="text-warning fw-bold mb-0">#<span class="row-index"></span> Artículo</h6>
                <button type="button" class="btn btn-outline-danger btn-sm rounded-pill btn-eliminar-fila px-3" onclick="eliminarFila(${index})">
                    <i class="fas fa-trash-alt me-1"></i> Quitar
                </button>
            </div>
            <div class="row g-3">
                <!-- Live search de inventario -->
                <div class="col-12 col-md-6">
                    <label class="form-label modal-encargo-label">Seleccionar del Inventario</label>
                    <!-- Hidden que envía el id_producto al servidor (vacío = producto libre) -->
                    <input type="hidden" class="hidden-id-producto" name="productos[${index}][id_producto]" value="">
                    <div class="prod-autocomplete">
                        <input type="text"
                               class="form-control modal-encargo-control text-white prod-search-input"
                ...
                <div class="col-12 col-md-3">
                    <label class="form-label modal-encargo-label">SKU</label>
                    <input type="text" class="form-control modal-encargo-control text-white input-sku" name="productos[${index}][sku]" placeholder="Ej: TS-PROD-01" required>
                </div>
                <div class="col-12 col-md-3">
                    <label class="form-label modal-encargo-label">Nombre del Producto</label>
                    <input type="text" class="form-control modal-encargo-control text-white input-nombre" name="productos[${index}][nombre]" placeholder="Nombre del artículo" required>
                </div>

                <div class="col-6 col-md-2">
                    <label class="form-label modal-encargo-label">Cantidad</label>
                    <input type="number" class="form-control modal-encargo-control text-white input-cantidad" name="productos[${index}][cantidad]" min="1" value="1" onkeyup="recalcularMontos()" onchange="recalcularMontos()" required>
                </div>
                <div class="col-6 col-md-2">
                    <label class="form-label modal-encargo-label">Costo Proveedor ($)</label>
                    <input type="number" step="0.01" class="form-control modal-encargo-control text-white input-precio-proveedor" name="productos[${index}][precio_proveedor]" min="0" value="0.00" onkeyup="recalcularMontos()" onchange="recalcularMontos()" required>
                </div>
                <div class="col-6 col-md-2">
                    <label class="form-label modal-encargo-label">Margen (%)</label>
                    <input type="number" step="0.1" class="form-control modal-encargo-control text-white input-margen" name="productos[${index}][margen]" min="0" value="30" onkeyup="recalcularMontos()" onchange="recalcularMontos()">
                </div>
                <div class="col-6 col-md-3">
                    <label class="form-label modal-encargo-label">Venta Sugerido ($)</label>
                    <input type="text" class="form-control modal-encargo-control text-white-50 input-venta-sugerido" readonly value="$0.00">
                </div>
                <div class="col-12 col-md-3">
                    <label class="form-label modal-encargo-label">Venta Final ($)</label>
                    <input type="number" step="0.01" class="form-control modal-encargo-control text-white input-venta-final" name="productos[${index}][precio_venta_final]" min="0" value="0.00">
                </div>

                <!-- Opciones de actualización / registro de inventario -->
                <div class="col-12 d-flex flex-wrap gap-3 mt-2 text-white-50 small">
                    <div class="form-check checkbox-actualizar-inventario-div" id="div_act_${index}">
                        ...
                        <label class="form-check-label text-white-50" for="check_act_${index}">Actualizar precio de venta en el inventario</label>
                    </div>
                    <div class="form-check checkbox-registrar-nuevo-div" id="div_new_${index}" style="display: block;">
                        ...
                        <label class="form-check-label text-warning fw-bold" for="check_new_${index}">Registrar como nuevo en el catálogo de inventario</label>
                    </div>
                </div>

                <!-- Indicador de costo prorrateado -->
                <div class="col-12 text-end text-success small pt-1 border-top border-secondary border-opacity-10">
                    <span>Costo Real Unitario: <strong>$<span class="label-costo-real-unit">0.00</span></strong></span>
                    <span class="ms-3">Costo Real Total: <strong>$<span class="label-costo-real-total">0.00</span></strong></span>
                </div>
            </div>
        </div>
        `;
